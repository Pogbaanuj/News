...
def single_page(request, id_single):
    ...
    for singlenews in similar_news:
        wordset = wordset.union(set(singlenews.news_title.split(' ')))

    allnewsdict = []
    for singlenews in similar_news:
        tempworddict = dict.fromkeys(wordset, 0)
        for word in singlenews.news_title.split(' '):
            tempworddict[word] += 1
    ...
    tfbowlist = {}
    for i in range(0, len(similar_news)):
        tfbow = Algo.ComputeTF(allnewsdict[i], similar_news[i].news_title.split(' '))
